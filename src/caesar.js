...
const caesarModule = (function () {
  ...
  const caesarShift = (character, shift, forward = true) => {
    ...
    for (let i = shift; i > 0; i--) {
      if (index === 25 && forward) { //Wrap around and keep shifting if we reach a boundary
        index = 0;
        newChar = alphabet[index];
      } else if (index === 0 && !forward) { //Wrap around and keep shifting if we reach a boundary
        index = 25;
        newChar = alphabet[index];
      } else if (index != 0 && !forward) { //Shift as normal
        index -= 1;
        newChar = alphabet[index];
      } else { //Shift as normal
        index += 1;
        newChar = alphabet[index];
      }
    }
